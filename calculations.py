...
class CryptoHandler:
	...
	def _call_client(self, coin, date_start, date_end):
		dict_prices = self.client.candles(coin, start=self.start_time, end=self.end_time)
		if not dict_prices:
			raise ValueError(
	...
	def _convert_to_df(self, coin, date_start, date_end):
		...
		dict_prices = self._call_client(coin, date_start, date_end)
	...
	def average_by_month(self, coin, date_start, date_end):
		...
		df_gb = df_prc.groupby("Date")["close"].mean().round(2).reset_index(name="Average price ($)")
		print(df_gb)
		return df_gb
